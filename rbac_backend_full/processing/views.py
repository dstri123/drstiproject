...
# ─── IFC to FBX Conversion ────────────────────────────────────────────────────

class ConvertIFCToFBXView(APIView):
    """
    POST /api/v1/processing/tools/convert-ifc/
        Upload an .ifc file (multipart field: 'file').
        Pipeline: ifcopenshell -> GLB -> Blender (headless) -> FBX.
        Response: { success, message, file_id, output_file, element_count }

    GET /api/v1/processing/tools/convert-ifc/?file_id=<id>
        Download the converted FBX. The file is deleted after download.

    Restricted to the data_contributor role.
    """
    permission_classes = [IsAuthenticated]

    def post(self, request):
        from .conversion import convert_ifc_to_fbx, ConversionError

        user_role = getattr(request.user, 'role', None)
        if user_role != 'data_contributor':
            return Response(
                {'error': 'Only data_contributor role can access this endpoint'},
        ...
        if not file_obj.name.lower().endswith('.ifc'):
            return Response({'error': 'Only .ifc files are supported'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            file_id, output_file, element_count = convert_ifc_to_fbx(
                file_obj, log=lambda m: print(f"[IFC2FBX] {m}")
            )
        ...
        _audit(request, 'process_done',
               f'IFC converted to FBX: {file_obj.name} ({element_count} elements)')

        return Response({
            'success': True,
            'message': f'Converted {file_obj.name} ({element_count} elements) to {output_file}',
            'file_id': file_id,
    ...
    def get(self, request):
        """Download a converted FBX by file_id, then delete it."""
        from django.http import FileResponse
        from .conversion import get_converted_file, delete_converted_file

        file_id = request.query_params.get('file_id')
        file_path = get_converted_file(file_id)
        if not file_path:
            return Response(
        ...
        # Auto-delete shortly after the download response is served
        def cleanup():
            import time
            time.sleep(30)
            delete_converted_file(file_id)

        Thread(target=cleanup, daemon=True).start()
        return response


# ─── IFC to GLB (viewer streaming) ────────────────────────────────────────────

class IFCToGLBView(APIView):
    """
    POST /api/v1/processing/tools/ifc-to-glb/

    Converts an uploaded .ifc file to GLB and streams it back directly so the
    3D viewer can render IFC files with the standard GLTF loader. Unlike the
    FBX tool this is available to any authenticated user, since it only powers
    read-only viewing.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        from .conversion import ifc_to_glb, ConversionError
        ...
        # Hash the IFC bytes so identical content always maps to the same cached
        # GLB. The expensive ifcopenshell/Blender conversion then runs only once
        # per unique file — later viewer loads return the cached GLB instantly.
        # A new/latest IFC has different bytes -> different hash -> fresh convert.
        ...
        cache_dir = os.path.join(_settings.MEDIA_ROOT, "glb_cache")
        os.makedirs(cache_dir, exist_ok=True)
        cached_glb = os.path.join(cache_dir, digest + ".glb")

        # Cache hit — skip conversion entirely.
        if os.path.exists(cached_glb) and os.path.getsize(cached_glb) > 0:
            with open(cached_glb, 'rb') as f:
                glb_bytes = f.read()
            response = HttpResponse(glb_bytes, content_type='model/gltf-binary')
            response['Content-Length'] = len(glb_bytes)
            response['X-GLB-Cache'] = 'hit'
            return response

        work_dir = _tempfile.mkdtemp(prefix="ifc2glb_")
        try:
            ...
            ifc_to_glb(input_path, glb_path, lambda m: print(f"[IFC2GLB] {m}"))

            with open(glb_path, 'rb') as f:
                glb_bytes = f.read()

            # Persist to the cache for next time (write atomically via temp file).
            try:
                tmp_cache = cached_glb + ".tmp"
                with open(tmp_cache, 'wb') as f:
                    f.write(glb_bytes)
                os.replace(tmp_cache, cached_glb)
            except OSError as cache_err:
                print(f"[IFC2GLB] failed to cache GLB: {cache_err}")

            response = HttpResponse(glb_bytes, content_type='model/gltf-binary')
            response['Content-Length'] = len(glb_bytes)
            response['X-GLB-Cache'] = 'miss'
            return response
        except ConversionError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
