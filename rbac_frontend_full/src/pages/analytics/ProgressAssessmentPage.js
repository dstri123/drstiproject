...
function Donut({ completed, inProgress, notStarted, size = 132 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={cx} cy={cx} r={r} fill="none" stroke="#eef2f6" strokeWidth={14} />
      {segs.map((s, i) => {
...
export default function ProgressAssessmentPage() {
  ...
  const pairPageCount = Math.max(1, Math.ceil(filteredPairs.length / PAIRS_PER_PAGE));
  ...
  const elementTypes = useMemo(() => {
    ...
    return Array.from(new Set(result.elements.map((e) => e.element_type))).sort();
  }, [result]);
  const filteredElements = useMemo(() => {
    if (!result?.elements) return [];
    return typeFilter === "ALL"
      ? result.elements
      : result.elements.filter((e) => e.element_type === typeFilter);
  }, [result, typeFilter]);
                ...
                >
                  ...
                  {pairs.map((pr) => (
                    <option key={pr.id} value={pr.id}>
                      BIM {fmtDate(pr.bim_date)} ↔ PC {fmtDate(pr.pointcloud_date)}
                      {pr.fitness != null
            ...
            {analyzing && (
              ...
              >
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 10 }}>
                  ...
                  surface coverage per element. Large clouds / many elements take
                  longer.
                </div>
                <style>{`
                ...
                {result.categories?.length > 0 && (
                  <Section title="By Category">
                    <table style={tableStyle}>
                      <thead>
                        <tr>
                          {["Category", "Count", "BIM Area (m²)", "Overlap (m²)", "Completion"].map(
                            (h) => (
                              <th key={h} style={thStyle}>
                                {h}
                              </th>
                          ...
                          <tr key={c.category}>
                            ...
                            <td style={tdStyle}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                ...
                                <span style={{ width: 38 }}>{c.completion}%</span>
                              </div>
                            </td>
                          </tr>
                        ))}
                ...
                >
                  <div style={{ maxHeight: 380, overflowY: "auto" }}>
                    <table style={tableStyle}>
                      <thead>
                        <tr>
                          {["Element ID", "Type", "BIM Area", "Overlap", "Completion", "Status"].map(
                            (h) => (
                              <th key={h} style={thStyle}>
                                {h}
                              </th>
                            ...
                            <tr key={e.element_id + i}>
                              ...
                              <td style={tdStyle}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                  ...
                                  <span style={{ width: 38 }}>{e.completion}%</span>
                                </div>
                              </td>
                              <td style={tdStyle}>
                                <span
